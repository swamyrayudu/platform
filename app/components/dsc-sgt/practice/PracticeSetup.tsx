...
export default function PracticeSetup({
  ...
  useEffect(() => {
    ...
    // Profile first, then the last manual switch — the same order the mock
    // tests tabs use. See lib/medium-preference.ts.
    const targetMedium = resolvePreferredMedium(user?.educationMedium)
      ...
      <div className="rounded-3xl border border-border/80 bg-card p-5 sm:p-7 shadow-xs space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {isPremium && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-500/20 to-orange-500/20 px-2.5 py-0.5 text-[11px] font-semibold text-amber-700 dark:text-amber-300 border border-amber-500/30">
          ...
          <button
            ...
            className="inline-flex min-h-11 cursor-pointer items-center gap-1.5 rounded-xl border border-border bg-card px-3.5 text-xs font-semibold text-foreground transition hover:bg-accent sm:min-h-0 sm:py-2"
          >
